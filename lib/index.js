...
const parseZodUnionDef = (def, refs) => {
    refs.addDataFunctions.addDescriptionIfExists(def, refs);
    return Object.assign({ type: "union", values: def.options.map((o) => zodSelectorFunction(o._def, Object.assign(Object.assign({}, refs), { path: [] }))) }, nodePropertiesFromRef(refs));
};
...
const zodSelectorFunction = (def, references) => {
    switch (def.typeName) {
        ...
        case zod.ZodFirstPartyTypeKind.ZodUnion:
            return parseZodUnionDef(def, references);
...
function nodeAndInputSchemaFromInputs(inputs, _routerPath, options, addDataFunctions) {
    if (!inputs.length) {
        return {
            ...
            schema: zodToJsonSchema.zodToJsonSchema(emptyZodObject, { errorMessages: true }),
    ...
    let input = inputs[0];
    if (inputs.length !== 1) {
        input = inputs.reduce((acc, input) => acc.merge(input), emptyZodObject);
    }
    return {
        parseInputResult: "success",
